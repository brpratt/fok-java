...
public class DiceRollOperatorApplication {
  ...
  public static class AppConfig {
    ...
    public CommandLineRunner runner(
        SharedInformerFactory sharedInformerFactory, Controller diceRollController) {
      return args -> {
    ...
    public GenericKubernetesApi<DiceRoll, DiceRollList> diceRollApi(ApiClient apiClient) {
      return new GenericKubernetesApi<>(
          DiceRoll.class, DiceRollList.class, "example.com", "v1", "dicerolls", apiClient);
    }

    @Bean
    public SharedIndexInformer<DiceRoll> podInformer(
    ...
    @Bean
    public Controller diceRollController(
        SharedInformerFactory sharedInformerFactory, DiceRollReconciler reconciler) {
      return ControllerBuilder.defaultBuilder(sharedInformerFactory)
          .watch(
              (q) -> {
                return ControllerBuilder.controllerWatchBuilder(DiceRoll.class, q).build();
              })
